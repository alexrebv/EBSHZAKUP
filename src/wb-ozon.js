...
/**
 * Последняя дата страницы — если по этой дате страница не убывает и вся лежит
 * не раньше начала окна. Два условия вместе означают, что метод и сортирует, и
 * фильтрует по этому самому полю: тогда всё, что раньше последней даты, он уже
 * отдал, и значение годится в курсор для `filter.since`.
 *
 * Проверка «не раньше начала окна» тут главная. Без неё можно сдвинуть `since`
 * по created_at, когда метод фильтрует по in_process_at: created_at у отправления
 * не позже in_process_at, поэтому такие страницы начинались бы раньше границы
 * окна — и отправления между двумя датами пропадали бы молча.
 */
function датаКурсора(пачка, начало) {
  for (const поле of ДАТЫ_ОТПРАВЛЕНИЯ) {
    let прошлая = -Infinity;
    let годится = true;
    for (const о of пачка) {
      const т = Date.parse(о[поле] ?? '');
      if (!Number.isFinite(т) || т < прошлая || т < начало) { годится = false; break; }
      прошлая = т;
    }
    if (годится && Number.isFinite(прошлая)) return прошлая;
  }
  return null;
}
 ...
 *    граница прочитанного, и `filter.since` сдвигается на неё.
 * 3. Делёж окна пополам. `since` и `to` метод соблюдает — у FBS они и вовсе
 *    обязательные, — поэтому половина окна это заведомо другая выборка. Правая
 *    половина ждёт в очереди.
 *
 ...
 *
 * Границу прочитанного двигает только первая страница окна: с курсором в руках
 * обход либо листает им, либо начинает окно заново. Повторы на стыках снимает
 * указатель номеров отправлений, поэтому осечка любого механизма стоит лишний
 * запрос, но не теряет отправления.
 *
...
export async function ozonОтправления(метод, задача, кластер) {
  ...
  for (;;) {
    ...
    const дата = датаКурсора(пачка, гр);
    ...
    // 3. делёж окна. Правая половина начинается ровно на середине, а не следующей
    // миллисекундой: если метод понимает `to` строго, отправление на шве не выпадет
